...
function Bestselling() {
    ...
    return (
        <Box display={"flex"}  boxShadow="rgba(0, 0, 0, 0.16) 0px 10px 36px 0px, rgba(0, 0, 0, 0.06) 0px 0px 0px 1px" p='1' bg='white' >

            <Box width="25%" h="330px" m="auto" textAlign={"center"} className="dealsBox">
                <Text paddingTop={{base:"120px", md:"100px", lg:"70px"}} fontSize={{ base: '10px', md: '20px', lg: '30px' }}> Constitutional Law </Text>
                ...
                <Img   src="https://www.lloydlawcollege.edu.in/images/LLC/website/images/blog_img/constitutional-law.webp" alt="fg" />

            </Box>
            <Box w={{ base: '80%', md: '75%', lg: '84%' }} m="auto" className="OffSlider">
                <Slider {...settings}>
              
                    {bestselling.filter(item=>item.catId===2).map((item, index) =>
                        <NavLink to={`/products/view/${item.id}`} key={Date.now()+index+Math.random()}>
                            <Box m="5px" alignItems="center" textAlign={"center"} >
                            <Img maxWidth="190px" h="240px" m="auto" _hover={{ transform: "scale(1.1)", transition: "400ms" }} p="10px" src={item.imagePath} alt="" />
              ...
              <Box  w="100%" display={{ base: 'block', md: 'none', lg: 'none' }} bg="orange" backgroundImage={"https://rukminim1.flixcart.com/fk-p-reco/850/200/images/Reco_BDS_e1c4ff.jpg?q=90"} p="10px" > 
                <Box mb="20px" mt="30px" alignItems={"center"} display="flex" justifyContent={"space-between"}> <Text fontWeight={"500"} fontSize="19px">  Constitutional Law</Text> <Button size="sm" colorScheme='messenger'>View All</Button></Box>
                ...
                {bestselling.filter(item=>item.catId===2).map((item, index) =>
                    <Box key={Math.random()+index} m="5px" borderRadius="6px" bg="white" alignItems="center" textAlign={"center"}border="1px solid silver">
                        <Img maxWidth="160px" h="190px" m="auto" _hover={{ transform: "scale(1.1)", transition: "400ms" }} p="10px" src={item.imagePath} alt="" />
